...
export const requestsAPI = {
  ...
  getLatest: (limit = 6, region?: string, province?: string, from?: string, to?: string) =>
    api.get('/requests', { params: { status: 'waiting', limit, ...(region ? { region } : {}), ...(province ? { province } : {}), ...(from ? { from } : {}), ...(to ? { to } : {}) } }),
